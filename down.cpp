...
{
public:
void add_to_file(int, int, int, int, int, float);    // Add record for balance cassette
...
// cassette code
void Update_balance(int, int, int , int , int, float); // Function to update balance after cassette sale
...
a.clear(5, 23);
gotoxy(5, 23);
cprintf(“\7Enter valid cassette number”);
getch();
gotoxy(5, 23);
cout << “Enter Total number of cassettes purchased”;
...
valid = 0;
gotoxy(5, 23);
cprintf(“\7Enter valid price for cassette”);
getch();
}
}while (!valid);
a.clear(5, 23);
...
valid = 0;
gotoxy(5, 23);
cprintf(“\7The availability of cassette is not sufficient”);
getch();
}
}while (!valid);
clear(5, 23);
